...
function buildLoad(){
  const lines=[
    ...
    "  const optional=async url=>{try{return await get(url)}catch(e){console.warn('Optional V5 source unavailable:',url,e);return null}};",
    '  try{',
    "    const [s,q,p,b,x,d]=await Promise.all([get(DATA+'?v=5.10'),get(QB+'?v=5.10'),get(PAPERS+'?v=5.10'),get(BOOKS+'?v=5.10'),optional('./data/content-expansion-v5.9.1.json?v=5.10'),optional('./data/deep-content-v5.10-schrodinger.json?v=5.10')]);",
