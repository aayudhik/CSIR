...
function makeTopic([sid,subject,topic,formula],i){
 ...
 const stems=[
  ...
  [`Which check is most useful after a calculation in ${topic}?`,['Dimensions, signs and a limiting case','Only decimal precision','Only the final line','No check is needed'],'Independent consistency checks catch many CSIR-NET errors.','Easy','Consistency'],
  [`Which statement about assumptions in ${topic} is correct?`,['The assumptions define where the result is valid','Assumptions never affect the answer','Assumptions can always be ignored','Only numerical values matter'],'Validity is determined by the model assumptions.','Moderate','Concept'],
  [`A Part C style extension of ${topic} should test:`,['A new situation requiring the same principles','Only verbatim recall','A copied final answer','No equations'],'Transfer and synthesis are central to higher-level questions.','Hard','Application'],
  [`When two methods are available for ${topic}, prefer the method that:`,['Makes assumptions and approximations explicit','Hides intermediate steps','Avoids checking limits','Uses more algebra automatically'],'Transparent reasoning is safer and easier to verify.','Moderate','Method'],
  [`What is the best revision prompt for ${topic}?`,['State the model, derive the key relation, give one application and one limiting case','Memorize the title only','Skip the derivation','Read without solving'],'A compact retrieval sequence reinforces conceptual and quantitative mastery.','Easy','Revision']
 ];
